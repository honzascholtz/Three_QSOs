...
ax.imshow(galaxy, vmax=1, origin='lower', interpolation='nearest', extent=(-szs/2,szs/2,-szs/2,szs/2), cmap=cmap)
ax.set_ylabel(r'$\Delta$Y (pixels)')
ax.set_ylim(-lms,lms)
ax.set_xlim(-lms,lms)

ax.text(-6,5, 'PSF', color='w')

ax.set_title('Model - map')

row=1
ax = h.add_axes((corner_x+0.5,corner_y-row*height_off, 0.17,height))
ax.imshow(galaxy_hole,vmax=0.6,  origin='lower', interpolation='nearest', extent=(-szs/2,szs/2,-szs/2,szs/2),cmap=cmap)
ax.set_ylabel(r'$\Delta$Y (pixels)')
ax.set_ylim(-lms,lms)
ax.set_xlim(-lms,lms)

ax.text(-6,5, 'Donut', color='w')

row=2
ax = h.add_axes((corner_x+0.5,corner_y-row*height_off, 0.17,height))
ax.imshow(galaxy_bar, vmax=0.6, origin='lower', interpolation='nearest', extent=(-szs/2,szs/2,-szs/2,szs/2), cmap=cmap)
ax.set_ylabel(r'$\Delta$Y (pixels)')
ax.set_xlabel(r'$\Delta$X (pixels)')
ax.set_ylim(-lms,lms)
ax.set_xlim(-lms,lms)

ax.text(-6,5, 'Arc', color='w')
...
ax.text(szs/6, 0.3, 'PSF')

ax.set_title('Model - COG')
...
ax.text(szs/6, 0.3, 'Arc')

ax.set_xlabel('Radius (pixels)')

    
plt.savefig(PATH+'Four_Quasars/Graphs/Paper_plots/Fig4_Halpha_COG.pdf', bbox_inches = 'tight')
