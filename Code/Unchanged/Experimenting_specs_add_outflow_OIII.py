...
o3_off = np.array([-577.,-229., -102. ])
o3_fwhm = np.array([2000.,1989., 1028. ])

import Plotting_tools as emplot
import Fitting_tools as emfit
import IFU_tools_QSO as IFU

Plots =  PdfPages(PATH+'Four_quasars/Graphs/Spectra_model_plus_outflow_OIII.pdf')
...
# =============================================================================  
    ...
    if i==1:
        out, chi2 = emfit.fitting_Halpha_mul_outflow_OIII(wave,flux,error,z, broad=1)
        ...
        out, chi2 = emfit.fitting_Halpha_mul_HB89_outflow_OIII(wave,flux,error,z, broad=1)
        
        out_old, chi2old =   emfit.fitting_Halpha_mul_testing(wave,flux,error,z, broad=1)
        
    
    dbic = out.bic-out_old.bic - np.log(len( np.where((wave>(6562.8-600)*(1+z)/1e4)&(wave<(6562.8+600)*(1+z)/1e4))[0]))*5
    ...
    if i !=2:
        ax1.text(6320, 1.0*max(out.eval(x=wave)), r'$\delta$BIC= %.1f' %(dbic), fontsize=15)
    else:
        ax1.text(6320, 1.0*max(out.eval(x=wave)), r'$\delta$BIC= %.1f' %(-dbic), fontsize=15)
