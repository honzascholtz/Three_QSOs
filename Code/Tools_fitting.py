...
def fitting_Halpha_mul_Lorentzian(wave, fluxs, error,z, wvnet=1., decompose=np.array([1]), offset=0,init_sig=300., broad=1, cont=1):
    from lmfit.models import GaussianModel, LorentzianModel, LinearModel, QuadraticModel, PowerLawModel
    ...
    model = LinearModel()+ LorentzianModel(prefix='Haw_') + GaussianModel(prefix='Han_') + GaussianModel(prefix='Nr_') + GaussianModel(prefix='Nb_')
    ...
    if len(decompose) == 1:
        parameters['Haw_sigma'].set(min=(2000.0/2.36/2.9979e5)*6562.8*(1+z)/1e4,max=(9000.0/2.36/2.9979e5)*Hal_cm) 
        parameters['Haw_center'].set(min=Hal_cm+ Hal_cm*(-400.0/2.9979e5),max=Hal_cm+ Hal_cm*(400.0/2.9979e5))
...
def fitting_CO(wave, fluxs, error, cent, initf = 500., chi2=0):
    from lmfit.models import LinearModel, GaussianModel
    ...
    peak_loc = np.ma.argmax(flux)
    peak = np.ma.max(flux)
    wave_peak = wave[peak_loc]
    
    
    wv = wave[np.where(wave==wave_peak)[0]] 
    
    
    model =  GaussianModel(prefix='CO_')
    ...
    parameters = model.make_params( \
		...
		CO_center = cent  ,         \
		#  [O III] 5007 sigma; start = 300 km/s FWHM			
		CO_sigma = (initf/2.36/2.9979e5)*cent, 
        )
    
    # Parameters constraints Narrow line flux > 0.0
    parameters['CO_amplitude'].set(min=0.0)
    parameters['CO_height'].set(max=peak)
    # Narrow line FWHM > min resolution of grating of KMOS (R >~ 3000) < max of 2500 km/s
    parameters['CO_sigma'].set(min=(100.0/2.36/2.9979e5)*cent,max=(1500.0/2.36/2.9979e5)*cent) 
    # Velocity offsets between -500 and 500 km/s for narrow
    parameters['CO_center'].set(min=cent + cent*(-900.0/2.9979e5),max=cent + cent *(900.0/2.9979e5))

    out = model.fit(flux,params=parameters, errors=error, x=(wave))
    
    chi2 = sum(((out.eval(x=wave)- flux)**2)/(error**2))/len(flux)
    
    return out, chi2
    
     
def fitting_CO_mu(wave, fluxs, error, cent, init=[200.,300.],chi2=0):
    from lmfit.models import LinearModel, GaussianModel
    ...
    model =  GaussianModel(prefix='COn_') +GaussianModel(prefix='COb_')
    

    parameters = model.make_params( \
                                   
		#  [O III] 5007 amplitude; start = 5 sigma of spectrum			
		COn_amplitude = peak/2,  \
		#  [O III] 5007 peak; start = zero offset, BLR 5 Ang offset to stagger			
		COn_center = cent[0]  ,         \
		#  [O III] 5007 sigma; start = 300 km/s FWHM			
		COn_sigma = (init[0]/2.36/2.9979e5)*cent[0], 
        
        #  [O III] 5007 amplitude; start = 5 sigma of spectrum			
		COb_amplitude = peak/2,  \
		#  [O III] 5007 peak; start = zero offset, BLR 5 Ang offset to stagger			
		COb_center = cent[1]  ,         \
		#  [O III] 5007 sigma; start = 300 km/s FWHM			
		COb_sigma = (init[1]/2.36/2.9979e5)*cent[0], 
        )
    
    # Parameters constraints Narrow line flux > 0.0
    parameters['COn_amplitude'].set(min=0) 
    # Narrow line FWHM > min resolution of grating of KMOS (R >~ 3000) < max of 2500 km/s
    parameters['COn_sigma'].set(min=(100.0/2.36/2.9979e5)*cent[0],max=(2000.0/2.36/2.9979e5)*cent[0]) 
    # Velocity offsets between -500 and 500 km/s for narrow
    parameters['COn_center'].set(min=cent[0] + cent[0]*(-1000.0/2.9979e5),max=cent[0] + cent[0] *(1000.0/2.9979e5))
    
    # Parameters constraints Broad line flux > 0.0
    parameters['COb_amplitude'].set(min=0.0) 
    # Narrow line FWHM > min resolution of grating of KMOS (R >~ 3000) < max of 2500 km/s
    parameters['COb_sigma'].set(min=(100.0/2.36/2.9979e5)*cent[0],max=(2000.0/2.36/2.9979e5)*cent[0]) 
    # Velocity offsets between -500 and 500 km/s for narrow
    parameters['COb_center'].set(min=cent[0] + cent[0]*(-1000.0/2.9979e5),max=cent[0] + cent[0] *(1000.0/2.9979e5))
...
def fitting_CO_muz(wave, fluxs, error, cent, init=[200.,300.],chi2=0):
    from lmfit.models import LinearModel, GaussianModel
    ...
    parameters['COb_sigma'].set(min=(100.0/2.36/2.9979e5)*cent[0],max=(2000.0/2.36/2.9979e5)*cent[0]) 
    # Velocity offsets between -500 and 500 km/s for narrow
    parameters['COb_center'].set(expr='COn_center')
...
def fitting_CO_double(wave, fluxs, error, cent, init=[200.,300.],chi2=0):
    from lmfit.models import LinearModel, GaussianModel
    ...
    parameters['COn_sigma'].set(min=(100.0/2.36/2.9979e5)*cent[0],max=(2000.0/2.36/2.9979e5)*cent[0]) 
    # Velocity offsets between -500 and 500 km/s for narrow
    parameters['COn_center'].set(min=cent[0] + cent[0]*(0.0/2.9979e5),max=cent[0] + cent[0] *(1000.0/2.9979e5))
    
    # Parameters constraints Broad line flux > 0.0
    parameters['COb_amplitude'].set(expr='COn_amplitude') 
    # Narrow line FWHM > min resolution of grating of KMOS (R >~ 3000) < max of 2500 km/s
    parameters['COb_sigma'].set(expr='COn_sigma') 
    # Velocity offsets between -500 and 500 km/s for narrow
    parameters['COb_center'].set(min=cent[0] + cent[0]*(-1000.0/2.9979e5),max=cent[0] + cent[0] *(0/2.9979e5))
...
def rot(x, CO_height , CO_center , CO_sigma, CO_rot):
    
    rotf = CO_rot/3e5*CO_center
    
    CO_sigma = CO_sigma/3e5*CO_center
    
     
    centera = CO_center-rotf
    centerb = CO_center+rotf
            
    a = gaussian(x, 1, centera, CO_sigma)
    b = gaussian(x, 1, centerb, CO_sigma)
...
RotModel = lmfit.Model( rot)


def rotation_fitting(wave, fluxs, error, cent):
    ...
    parameters['CO_height'].set(min=0)
      
    parameters['CO_sigma'].set(min= 100.,max=1000) 
    parameters['CO_rot'].set(min=50,max=1000.0) 
    # Velocity offsets between -500 and 500 km/s for narrow
    parameters['CO_center'].set(min=cent + cent*(-500.0/2.9979e5),max=cent + cent *(500.0/2.9979e5))
...
def fitting_Halpha_mul_testing(wave, fluxs, error,z, wvnet=1., decompose=1, offset=0,init_sig=300., broad=1, cont=1):
    from lmfit.models import GaussianModel, LorentzianModel, LinearModel, QuadraticModel, PowerLawModel
    ...
    model = LinearModel()+ lmfit.Model(Gaussian_BK, prefix='Haw_') + GaussianModel(prefix='Han_') + GaussianModel(prefix='Nr_') + GaussianModel(prefix='Nb_') + GaussianModel(prefix='X_') + GaussianModel(prefix='SII_')
    ...
    parameters = model.make_params( \
      #  Continuum level @ 5000 Ang; start = mean flux of spectrum			
		...
		Han_sigma = (init_sig/2.36/2.9979e5)*Hal_cm, \
         ...
         SII_amplitude = peak/6, \
             
         SII_center = 6730.*(1+z)/1e4, \
         SII_sigma = (1000/2.36/2.9979e5)*Hal_cm, \
    ...
    if decompose == 1:
        parameters['Haw_sigma'].set(min=(2000.0/2.36/2.9979e5)*6562.8*(1+z)/1e4,max=(12000.0/2.36/2.9979e5)*Hal_cm) 
        parameters['Haw_center'].set(min=Hal_cm+ Hal_cm*(-1500.0/2.9979e5),max=Hal_cm+ Hal_cm*(2500.0/2.9979e5))
        ...
        parameters['SII_amplitude'].set(min=0) 
        #parameters['Hao_amplitude'].set(max=2) 
        parameters['SII_center'].set(min=6730.*(1+z)/1e4+ Hal_cm*(-500.0/2.9979e5),max=6730.*(1+z)/1e4+ Hal_cm*(500.0/2.9979e5))
     
        parameters['SII_sigma'].set(min=(1000.0/2.36/2.9979e5)*6562.8*(1+z)/1e4,max=(3000.0/2.36/2.9979e5)*Hal_cm) 
        ...
        parameters['SII_sigma'].set(min= 0.999*outo.params['SII_sigma'],max=1.0001*outo.params['SII_sigma']) 
        parameters['SII_center'].set(min= 0.999*outo.params['SII_center'],max=1.0001*outo.params['SII_center']) 
...
def fitting_Halpha_mul_LBQS_testing(wave, fluxs, error,z, wvnet=1., decompose=1, offset=0,init_sig=300., broad=1, cont=1, cont_norm='n'):
    from lmfit.models import GaussianModel, LorentzianModel, LinearModel, QuadraticModel, PowerLawModel
    ...
    model = LinearModel()+ lmfit.Model(Gaussian_BK, prefix='Haw_') + GaussianModel(prefix='Han_') + GaussianModel(prefix='Nr_') + GaussianModel(prefix='Nb_') + GaussianModel(prefix='X_') + GaussianModel(prefix='Hao_')
    ...
    parameters = model.make_params( \
      #  Continuum level @ 5000 Ang; start = mean flux of spectrum			
		...
		Han_sigma = (init_sig/2.36/2.9979e5)*Hal_cm, \
         ...
         Hao_amplitude = peak/2, \
             
         Hao_center = 6543.*(1+z)/1e4, \
         Hao_sigma = (1000/2.36/2.9979e5)*Hal_cm, \
    ...
    if decompose == 1:
        ...
        #parameters['Hao_amplitude'].set(max=2) 
        parameters['Hao_center'].set(min=6543.*(1+z)/1e4+ Hal_cm*(-1500.0/2.9979e5),max=6543.*(1+z)/1e4+ Hal_cm*(600.0/2.9979e5))
    
    
    
        parameters['Hao_sigma'].set(min=(1000.0/2.36/2.9979e5)*6562.8*(1+z)/1e4,max=(3000.0/2.36/2.9979e5)*Hal_cm)   
        ...
        parameters['Hao_sigma'].set(min= 0.999*outo.params['Hao_sigma'],max=1.0001*outo.params['Hao_sigma']) 
        parameters['Hao_center'].set(min= 0.999*outo.params['Hao_center'],max=1.0001*outo.params['Hao_center']) 
    ...
    parameters['Hao_amplitude'].set(min=0) 
...
def fitting_Halpha_mul_outflow(wave, fluxs, error,z, wvnet=1., decompose=1, offset=0,init_sig=300., broad=1, cont=1, cont_norm='n'):
    from lmfit.models import GaussianModel, LorentzianModel, LinearModel, QuadraticModel, PowerLawModel
    ...
    model = LinearModel()+ lmfit.Model(Gaussian_BK, prefix='Haw_') + GaussianModel(prefix='Han_') + GaussianModel(prefix='Nr_') + GaussianModel(prefix='Nb_') + GaussianModel(prefix='X_') + GaussianModel(prefix='Hao_') + GaussianModel(prefix='Nro_') + GaussianModel(prefix='Nbo_')
    ...
    parameters = model.make_params( \
      #  Continuum level @ 5000 Ang; start = mean flux of spectrum			
		...
		Han_sigma = (init_sig/2.36/2.9979e5)*Hal_cm, \
         ...
         Hao_amplitude = peak/20, \
             
         Hao_center = Hal_cm, \
         Hao_sigma = (1000/2.36/2.9979e5)*Hal_cm, \
             
         Nro_amplitude = peak/25, \
         #
         Nro_center = 6583.*(1+z)/1e4, \
         #
         Nro_sigma = (1000/2.36/2.9979e5)*Hal_cm, \
         #
         Nbo_amplitude = peak/75, \
         #
         Nbo_center = 6548.*(1+z)/1e4, \
         #
         Nbo_sigma = (1000/2.36/2.9979e5)*Hal_cm, \
    ...
    if decompose == 1:
        ...
        #parameters['Hao_amplitude'].set(max=2) 
        parameters['Hao_center'].set(min=Hal_cm + Hal_cm*(-1500.0/2.9979e5),max=Hal_cm + Hal_cm*(600.0/2.9979e5))
    
    
    
        parameters['Hao_sigma'].set(min=(500.0/2.36/2.9979e5)*6562.8*(1+z)/1e4,max=(2000.0/2.36/2.9979e5)*Hal_cm)   
    ...
    parameters['Nro_amplitude'].set(min=0.0)
    #parameters['Nr_amplitude'].set(expr = 'Han_amplitude/1000000000')
    #
    parameters['Nbo_amplitude'].set(expr='Nro_amplitude/3')  
    #
    parameters['Nro_sigma'].set(expr='Hao_sigma*(6583/6562)')
    #
    parameters['Nbo_sigma'].set(expr='Hao_sigma*(6548/6562)') 
    
    offset_r = (6562.-6583.)*(1+z)/1e4
    #
    parameters['Nro_center'].set(expr='Hao_center - '+str(offset_r))
    
    offset_b = (6562.-6548.)*(1+z)/1e4
    #
    parameters['Nbo_center'].set(expr='Hao_center - '+str(offset_b))
...
def fitting_Halpha_mul_HB89_outflow(wave, fluxs, error,z, wvnet=1., decompose=1, offset=0,init_sig=300., broad=1, cont=1, cont_norm='n'):
    from lmfit.models import GaussianModel, LorentzianModel, LinearModel, QuadraticModel, PowerLawModel
    ...
    model = LinearModel()+ lmfit.Model(Gaussian_BK, prefix='Haw_') + GaussianModel(prefix='Han_') + GaussianModel(prefix='Nr_') + GaussianModel(prefix='Nb_') + GaussianModel(prefix='X_') + GaussianModel(prefix='SII_') + GaussianModel(prefix='Hao_') + GaussianModel(prefix='Nro_') + GaussianModel(prefix='Nbo_')
...
def fitting_Halpha_pr_sub_narrow(wave, fluxs, error,z, wvnet=1., decompose=1, offset=0,init_sig=300., broad=1, cont=1):
    from lmfit.models import GaussianModel, LorentzianModel, LinearModel, QuadraticModel, PowerLawModel
    ...
    parameters = model.make_params( \
      #  Continuum level @ 5000 Ang; start = mean flux of spectrum			
		...
		Han_sigma = (init_sig/2.36/2.9979e5)*Hal_cm, \
         ...
         Hao_amplitude = peak*(1./6),  \
		 #  			
		 Hao_center = wv,         \
		#  	
		 Hao_sigma = (500/2.36/2.9979e5)*Hal_cm, \
	)
    if cont==0:
        parameters['intercept'].set(min=-0.0000000000001) 
        parameters['intercept'].set(max= 0.0000000000001) 
    ...
    parameters['Hao_amplitude'].set(min=0.0) 
    parameters['Hao_sigma'].set(min=((200.0/2.9979e5)*Hal_cm),max=(800/2.36/2.9979e5)*Hal_cm) 
    parameters['Hao_center'].set(min=wvnet+ Hal_cm*(-600.0/2.9979e5),max=wvnet+ Hal_cm*(600.0/2.9979e5))
    #
    parameters['Nb_center'].set(expr='Han_center - '+str(offset_b))
    #parameters['Haw_sigma'].set(min=(2000.0/2.36/2.9979e5)*6562.8*(1+z)/1e4,max=(2500.0/2.36/2.9979e5)*Hal_cm) 
    parameters['X_center'].set(min=6361*(1+z)/1e4+ Hal_cm*(-900.0/2.9979e5),max=6361*(1+z)/1e4+ Hal_cm*(900.0/2.9979e5))
...
def fitting_Halpha_mul_HB89_narrow(wave, fluxs, error,z, wvnet=1., decompose=1, offset=0,init_sig=300., broad=1, cont=1):
    from lmfit.models import GaussianModel, LorentzianModel, LinearModel, QuadraticModel, PowerLawModel
    ...
    model = LinearModel()+ lmfit.Model(Gaussian_BK, prefix='Haw_') + GaussianModel(prefix='Han_') + GaussianModel(prefix='Nr_') + GaussianModel(prefix='Nb_') + GaussianModel(prefix='X_') + GaussianModel(prefix='SII_')+  GaussianModel(prefix='Hao_')
...
def fitting_Halpha_mul_outflow_1G(wave, fluxs, error,z, wvnet=1., decompose=1, offset=0,init_sig=300., broad=1, cont=1, cont_norm='n'):
    from lmfit.models import GaussianModel, LorentzianModel, LinearModel, QuadraticModel, PowerLawModel
    ...
    if decompose == 1:
        ...
        #parameters['Hao_amplitude'].set(max=2) 
        parameters['Hao_center'].set(min=Hal_cm + Hal_cm*(-1600.0/2.9979e5),max=Hal_cm + Hal_cm*(600.0/2.9979e5))
    
    
    
        parameters['Hao_sigma'].set(min=(500.0/2.36/2.9979e5)*6562.8*(1+z)/1e4,max=(1600.0/2.36/2.9979e5)*Hal_cm)   
...
def fitting_Halpha_mul_HB89_outflow_1G(wave, fluxs, error,z, wvnet=1., decompose=1, offset=0,init_sig=300., broad=1, cont=1, cont_norm='n'):
    from lmfit.models import GaussianModel, LorentzianModel, LinearModel, QuadraticModel, PowerLawModel
...
def fitting_Halpha_mul_outflow_OIII(wave, fluxs, error,z, wvnet=1., decompose=1, offset=0,init_sig=300., broad=1, cont=1, cont_norm='n', o3off=1, o3fwhm=1):
    from lmfit.models import GaussianModel, LorentzianModel, LinearModel, QuadraticModel, PowerLawModel
    ...
    if decompose == 1:
        ...
        #parameters['Hao_center'].set(expr= str(Hal_cm+o3off/3e5*Hal_cm))
        
        parameters['Hao_center'].set(min= Hal_cm+ Hal_cm*((o3off-100)/2.9979e5),max=Hal_cm+ Hal_cm*((o3off+100)/2.9979e55))
        
        
        #parameters['Hao_fwhm'].set(expr=str(o3fwhm/3e5*Hal_cm))
        parameters['Hao_fwhm'].set( min=(o3fwhm-100)/3e5*Hal_cm, max=(o3fwhm+100)/3e5*Hal_cm)
        #parameters['Hao_sigma'].set(min=(500.0/2.36/2.9979e5)*6562.8*(1+z)/1e4,max=(2000.0/2.36/2.9979e5)*Hal_cm)   
...
def fitting_Halpha_mul_HB89_outflow_OIII(wave, fluxs, error,z, wvnet=1., decompose=1, offset=0,init_sig=300., broad=1, cont=1, cont_norm='n', o3off=1, o3fwhm=1):
    from lmfit.models import GaussianModel, LorentzianModel, LinearModel, QuadraticModel, PowerLawModel
    ...
    if decompose == 1:
        ...
        parameters['Haw_a2'].set(max=0.0)
        parameters['Haw_a2'].set(min= -slp_edge)
        #parameters['Haw_a2'].set(expr= '- Haw_a1')
