...
def check_plots(storage):
    # This should show emission lines, sky lines etc. 
    flux_line = np.ma.array(data=storage['flux'].data, mask= storage['flux'].mask) 
    wave =  storage['obs_wave']   

    f, (ax1,ax2,ax3) = plt.subplots(3,sharex=True, sharey=True)
    ax1.plot(wave, flux_line[:,6,8], drawstyle='steps-mid')
    ax1.set_title('Original - Mask - Just masked non Zeroes')

    # This should show skylines but no emission lines
    flux_line = np.ma.array(data=storage['flux'].data, mask= storage['em_line_mask']) 
    wave =  storage['obs_wave']   


    ax2.plot(wave, flux_line[:,6,8], drawstyle='steps-mid')
    ax2.set_title('Emission lines masked')


    # This should should mask skylines and emission lines
    flux_line = np.ma.array(data=storage['flux'].data, mask= storage['sky_clipped+em']) 
    wave =  storage['obs_wave']   

    ax3.plot(wave, flux_line[:,6,8], drawstyle='steps-mid')
    ax3.set_title('Sky lines and Emission line masked masked')
    plt.tight_layout()
 


def plotting_OIII(wave, fluxs, ax, out, mode,z, title=1):
    ...
    if mode=='mul':
        ax.plot(wv_rest[fit_loc], out.eval_components(x=wave[fit_loc])['o3rw_'], color= 'blue', linestyle ='dashed')
        ax.plot(wv_rest[fit_loc], out.eval_components(x=wave[fit_loc])['o3rn_'], color= 'green', linestyle ='dotted')
    
        ax.plot(wv_rest[fit_loc], out.eval_components(x=wave[fit_loc])['o3bw_'], color= 'green', linestyle ='dotted')
        ax.plot(wv_rest[fit_loc], out.eval_components(x=wave[fit_loc])['o3bn_'], color= 'blue', linestyle ='dashed')
        
        rs = 5006.84 *(1+z)/1e4
        broad = out.params['o3rw_fwhm'].value/rs*2.9979e5
        narow = out.params['o3rn_fwhm'].value/rs*2.9979e5
        
        if title==1:
            ax.set_title('B,N '+ str(int(broad))+', '+ str(int(narow))+' km/s'  )
        
        #ax.text(4900, max(flux)*0.8, 'Broad: '+ str(broad)+' km/s')
        #ax.text(4900, max(flux)*0.7, 'Narrow: '+ str(narow)+' km/s')
        
    elif mode=='sig':
        ax.plot(wv_rest[fit_loc], out.eval_components(x=wave[fit_loc])['o3r_'], 'b--')
        ax.plot(wv_rest[fit_loc], out.eval_components(x=wave[fit_loc])['o3b_'], 'b--')
...
def plotting_OIII_Hbeta(wave, fluxs, ax, out, mode,z, title=1, Hbeta=1, OIII=1):
    
    try:
        z =  (out.params['o3rn_center'].value*1e4/5008)-1
        
    except:
        z = (out.params['o3r_center'].value*1e4/5008)-1
    
    
    wv_rest = wave/(1+z)*1e4
    fit_loc = np.where((wv_rest>4100)&(wv_rest<5500))[0]
    ...
    fit_loc_sc = np.where((wv_rst_sc>4100)&(wv_rst_sc<5500))[0]   
    
    ax.plot(wv_rst_sc[fit_loc_sc],flux[fit_loc_sc], drawstyle='steps-mid')
    
    
    #try:  
    #    ax.plot(wv_rest[fit_loc],(out.eval_components(x=wave[fit_loc])['linear']), 'r--')
    #except:
    #     print('Continuum?')
    ...
    if (OIII==0) & (Hbeta==0):
        ...
        ax.plot(wv_rest[fit_loc], out.eval(x=wave[fit_loc])-(out.eval_components(x=wave[fit_loc])['o3bw_']+ out.eval_components(x=wave[fit_loc])['o3rw_']+out.eval_components(x=wave[fit_loc])['Hb_']), 'r--')
        
    elif Hbeta==0:
        fit_loc = np.where((wv_rest>4100)&(wv_rest<5900))[0]
        
        ax.plot(wv_rest[fit_loc], out.eval(x=wave[fit_loc])-out.eval_components(x=wave[fit_loc])['Hb_'], 'r--')
        ...
        ax.plot(wv_rest[fit_loc], out.eval(x=wave[fit_loc])-(out.eval_components(x=wave[fit_loc])['o3bw_']+ out.eval_components(x=wave[fit_loc])['o3rw_']), 'r--')
    ...
    if mode=='mul':
        ...
        ax.plot(wv_rest[fit_loc], out.eval_components(x=wave[fit_loc])['o3bn_'], color= 'green', linestyle ='dotted')
        ax.plot(wv_rest[fit_loc], out.eval_components(x=wave[fit_loc])['o3rn_'], color= 'green', linestyle ='dotted')
    
        if OIII==1:
            try:
                ax.plot(wv_rest[fit_loc], out.eval_components(x=wave[fit_loc])['o3bw_'], color= 'blue', linestyle ='dashed')
                ax.plot(wv_rest[fit_loc], out.eval_components(x=wave[fit_loc])['o3rw_'], color= 'blue', linestyle ='dashed')
            except:
                lsderr=1
        ...
        if Hbeta==1:
            ax.plot(wv_rest[fit_loc], out.eval_components(x=wave[fit_loc])['Hbn_'], linestyle='dashed', color='orange')
            ax.plot(wv_rest[fit_loc], out.eval_components(x=wave[fit_loc])['Hbw_'], linestyle='dotted', color='orange')
...
def save_obj(obj, name ):
    with open(ph.MyPATH + name + '.pkl', 'wb') as f:
        pickle.dump(obj, f, pickle.HIGHEST_PROTOCOL)

def load_obj(name ):
    import pickle
    with open(ph.MyPATH + name + '.pkl', 'rb') as f:
...
def new_ALMA_contour_plot(storage,ax, infos=1, linew =2.,respond=0, both=0, prj='triple', clr='red', linestyle='dashed'):
    ...
    import glob
    afile = glob.glob(ph.MyPATH+folder+'/ALMA/Final_set/'+ID+'_l*')
    
    if len(afile)>0:
        
        
        cont_file =glob.glob(ph.MyPATH+folder+'/ALMA/Final_set/'+ID+'_l_clean.pbcor.*')[0]        
    ...
    import glob
    afile = glob.glob(ph.MyPATH+folder+'/ALMA/Final_set/'+ID+'_h*')
...
def Spax_fit_plot_sig(storage, mode, z, ID, binning, instrument='KMOS',add='', addsave= '', prj='triple'):
    ...
    if instrument == 'KMOS':
        ...
        ins = '_sin'
    if mode =='OIII':       
        Image_narrow = pyfits.getdata(ph.MyPATH+'KMOS_SIN/Results_storage/OIII/'+ID+'_'+binning+'_spaxel_fit'+ins+add+'.fits')
        
    elif mode =='H':
        Image_narrow = pyfits.getdata(ph.MyPATH+'KMOS_SIN/Results_storage/Halpha/'+ID+'_'+binning+'_spaxel_fit'+ins+add+'.fits')
